...
| **Geshippte Tier**     | 0 (Foundation), 1 (Core-Mechaniken), 2 (Among-Us-Features), 3 (Mini-Games), 3.5 (Persona-Layer), 3.6 (Meeting-Kontext + AI-Flavor), 3.7 (Endscreen + Closing-Mini-Games + Metrik-Export), 3.8 (Map-Authoring-Toolchain — Editor + 3D-Vorschau + Floor-Textures + Door-Frames + Kinds-Registry + ASSET_SPEC + datacenter)                          |
| **In Arbeit**          | Tier 3.9 (AI-Integration) Slice 3.9.1 (Provider-Abstraction) + 3.9.2 (AI-NPCs Lobby-Filler) — dual provider Anthropic Haiku + lokales Gemma 4                                                                                                                                                                                                     |
...
| #     | Was                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Status         |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| 3.9.1 | **LLM-Provider-Abstraction** — `app/game/llm.py` mit `LLMClient` Protocol, `AnthropicClient` (anthropic SDK + Prompt-Caching auf System-Prompt) und `LocalOpenAIClient` (HTTP `/v1/chat/completions` für Ollama/Gemma 4). Provider-Selektion via `ANTHROPIC_API_KEY` oder `LLM_LOCAL_BASE_URL`+`LLM_LOCAL_MODEL`. 3 s Timeout, Fallback no-op (Tests + Prod ohne LLM laufen weiter).                                                                                                                                                                                                                        | ⏳ in progress |
| 3.9.2 | **AI-NPCs (Lobby-Filler-Bots, Release-Team)** — `app/game/bots/` mit BotManager + Pathfinding (BFS/A\* über room-graph aus Türen). LLM-Call alle 5 s pro Bot für High-Level-Intent („gehe in die Küche und mache coffee"), reactive Overrides für Body-in-Sight (sofort melden) / Meeting (Vote-Heuristik) / Sabotage (Repair-Path). Hold-E-Tasks: Bots skippen Mini-Game und auto-completen nach 4–6 s (Marker `bot_auto_complete` im Metrik-Export). Lobby-UI: Host-Button „Bot hinzufügen" (curated Names: Bot-Promptly, Bot-Cursor-Sr., Bot-StackOverflow, Bot-Junior). Phase 1: nur Release-Team-Bots. | ⏳ open        |
...
**Stand 2026-04-28:** Tier 4.0–4.5 + 4.13 sind durch — Asset-Pipeline mit 25/25 Kinds gestaged, Web-Export auf https://prod-is-lava.dev/godot/ live, Lobby+World+Character+HUD+Pause-Menü laufen. Feature-Parität zum 2D-JS-Client liegt aktuell bei ~30 % (Lobby-Connect, Movement, Map-Render, HUD-Stats), die fetten 70 % (Mini-Games, Sabotagen, Meeting+Voting, Among-Us-Features, Endscreen, Eventfeed, Role-Intro, Personal-Tasks) sind noch offen. Vollständige Onboarding-Doku unter [`docs/GODOT_HANDOFF.md`](GODOT_HANDOFF.md). Schema-Konsolidierung via `maps/kinds.json` ist durch (siehe Tier 3.8.7).

#### Feature-Parität-Matrix (JS-Client → Godot-Client, Stand 2026-04-28)
...
| Lobby-Map-Auswahl            | Dropdown aus `availableMaps`                 | hardcoded auf erste Map                                                                                | ⛔ Slice 4.2.1                    |
| Lobby-Rollen-Präferenz       | Dropdown 5 Release-Rollen                    | gar nicht                                                                                              | ⛔ Slice 4.2.2                    |
| Lobby-Bot-Buttons (Tier 3.9) | add_bot / remove_bot pro Klick               | gar nicht                                                                                              | ⛔ Slice 4.2.3                    |
...
| MapObject-Collision          | Server-side `_walls` enforced                | Server enforced, aber Client-Lerp (14.0) lässt Char visuell kurz in den Tisch laufen vor dem Snap-Back | 🐛 Slice 4.3.1 (visual smoothing) |
| TaskAnchor-Marker            | Floating Title + Status                      | Sphere mit Emission                                                                                    | ✅ done                           |
| Vents                        | Interactive Object + V-Key Cycle             | gar nicht                                                                                              | ⛔ Slice 4.10                     |
| Hold-to-Interact (E)         | E-Key, sendet `task_hold_*`                  | E-Key, sendet `task_hold_*` mit Proximity-Check                                                        | ✅ done                           |
| Mini-Game-Framework          | Modal + Registry, 8 Spiele                   | Modal + Registry, **1 Spiel** (sprint_trim)                                                            | 🟡 Slice 4.6 (7 fehlen)           |
| Personal-Task-Panel ★        | Sidebar mit eigenen Tasks markiert           | gar nicht                                                                                              | ⛔ Slice 4.5.1                    |
| Sabotage-Buttons             | 8 Icons + Cooldown-Ring + Object-Binding     | gar nicht                                                                                              | ⛔ Slice 4.7                      |
| Take-Down (Force-Reboot)     | Proximity-Button + War-Room-Safe             | gar nicht                                                                                              | ⛔ Slice 4.10                     |
| Body-Discovery + Report      | Proximity-Button mit Trigger                 | gar nicht                                                                                              | ⛔ Slice 4.10                     |
| Lights-Sabotage VFX          | Radial Vignette                              | gar nicht                                                                                              | ⛔ Slice 4.10                     |
| Comms-Sabotage VFX           | Sidebar leer + Buttons disabled              | gar nicht                                                                                              | ⛔ Slice 4.10                     |
| Spectator-Mode (Geist)       | Free-Move + sieht andere Geister             | Ghost-Alpha vorhanden, kein full-Mode                                                                  | 🟡 Slice 4.10                     |
| Meeting-Modal mit Context    | Reporter+Body+RecentEvents                   | Phase-Detection + Sting, kein UI                                                                       | ⛔ Slice 4.8                      |
| Voting-UI (cast/skip)        | Buttons pro Spieler + Skip                   | gar nicht                                                                                              | ⛔ Slice 4.8                      |
| Voting-Result + Last-Words   | Toast mit AI-Quote                           | gar nicht                                                                                              | ⛔ Slice 4.8                      |
| HUD-Stats-Pills (4)          | Release/Pipeline/Coffee/Incidents + Timer    | analoge Stat-Bars                                                                                      | ✅ done                           |
| HUD-Coffee-Bar (privat)      | private_state-bound, pulsiert <15            | private_state-bound, kein Pulse                                                                        | 🟡 Slice 4.5.2                    |
| HUD-Ability-Button           | Per-Rolle, 1×/Runde                          | gar nicht                                                                                              | ⛔ Slice 4.5.3                    |
| HUD-Eventfeed                | Live-Log rechts, AI-flavored                 | gar nicht                                                                                              | ⛔ Slice 4.5.4                    |
| Role-Intro-Modal             | Beim Phase-Wechsel, 30 s Auto-Dismiss        | gar nicht                                                                                              | ⛔ Slice 4.5.5                    |
| Endscreen                    | Awards + Per-Player-Stats + AI-Postmortem    | gar nicht                                                                                              | ⛔ Slice 4.9                      |
...
| Audio (BGM)                  | Optional, mute-toggle                        | bewusst ausgelassen                                                                                    | 🔮 Slice 4.11                     |
| In-Game-ESC-Menü             | Lobby-verlassen, Runde-beenden, Audio-Slider | Lobby-verlassen, Runde-beenden, kein Audio                                                             | 🟡 Slice 4.12                     |
| Touch-Controls (Mobile)      | Virtual joystick + 4 Action-Buttons          | gar nicht                                                                                              | ⛔ Slice 4.12                     |
| Reconnect bei DC             | Server bewahrt Identität 30 s                | Web-Build hat keine Reconnect-Logic                                                                    | ⛔ Slice 4.12                     |
| AI-NPC-Bots in Lobby         | add_bot / remove_bot Buttons                 | gar nicht (siehe Slice 4.2.3)                                                                          | ⛔ Slice 4.2.3                    |
...
| 4.2.1 | **Lobby — Map-Auswahl-Dropdown** ← Sven-Pain. `lobby_state.availableMaps` an OptionButton koppeln, Host-only-Edit, `select_map`-Send wie im JS-Client.                                                                                                                                                                                                                                                                                                                   | 0.5 Tag  | ⛔ TODO (next)                         |
| 4.2.2 | **Lobby — Rollen-Präferenz-Dropdown** ← Sven-Pain. 5 Release-Rollen + "Random" als OptionButton, `set_preferred_role`-Send. Chaos-Wunsch wird ignoriert (Server-side).                                                                                                                                                                                                                                                                                                   | 0.5 Tag  | ⛔ TODO (next)                         |
| 4.2.3 | **Lobby — AI-NPC-Buttons** (Tier-3.9-Bots) — `add_bot` / `remove_bot` für den Host, kuratierte Bot-Namen aus dem Server-Pool.                                                                                                                                                                                                                                                                                                                                            | 0.5 Tag  | ⛔ TODO                                |
...
| 4.5.1 | **HUD — Personal-Task-Panel mit ★** — Sidebar listet alle Tasks (room, status, progress), eigene Tasks bekommen ★ + amber Border. Quelle: `private_role.assignedTasks[]`.                                                                                                                                                                                                                                                                                                | 0.5 Tag  | ⛔ TODO                                |
| 4.5.2 | **HUD — Coffee-Bar Pulse < 15** — vorhandene Coffee-Bar bekommt rot+pulsing-Animation wenn `private_state.coffeeEnergy < 15` (Speed-Penalty-Threshold).                                                                                                                                                                                                                                                                                                                  | 0.25 Tag | ⛔ TODO                                |
| 4.5.3 | **HUD — Active-Ability-Button** — Per-Rolle (Coffee Run, Rollback, Standup, Reproduce Bug), 1×/Runde, sendet `use_ability`. Disabled wenn `private_state.abilityUsed=true` oder phase!=playing.                                                                                                                                                                                                                                                                          | 0.5 Tag  | ⛔ TODO                                |
| 4.5.4 | **HUD — Eventfeed** — RichTextLabel rechts, scrollt zu newest, Severity-Klassen (info/warn/error). Server-Events aus `game_state.events[]`. AI-flavored Texte (Tier 3.6/3.9).                                                                                                                                                                                                                                                                                            | 0.5 Tag  | ⛔ TODO                                |
| 4.5.5 | **Role-Intro-Modal** — beim phase=playing-Wechsel: Modal mit Team-Color, Rolle, Description, Strengths, Ability, AssignedTasks. 30 s Auto-Dismiss oder Click-to-close. Dedupe wenn role_id unverändert.                                                                                                                                                                                                                                                                  | 0.5 Tag  | ⛔ TODO                                |
| 4.6   | **Mini-Games-Sweep — 7 fehlende Spiele** — sprint_trim ist fertig. Portieren: test_suite_repair (sequencing), cable_pairing (drag-pair), coffee_pour (timing-stop), log_filter (multi-select), diff_review (filter-by-criterion), stability_balance (rotating-correction), release_notes (click-to-cycle-sort). Alle nutzen die existierende Modal+Registry-Architektur, nur die UI pro Spiel ist neu.                                                                   | 4–5 Tage | ⛔ TODO                                |
| 4.7   | **Sabotage-Buttons** — 8 Icons aus `images/sabotage_icons.png`, Cooldown-Ring + numeric, Object-Binding-Reichweiten-Check (60 px), Disabled-States (ghost, comms-down). Sendet `trigger_sabotage`.                                                                                                                                                                                                                                                                       | 1 Tag    | ⛔ TODO                                |
| 4.8   | **Meeting + Voting** — Meeting-Modal mit Context-Block (Reporter, Body+Room, RecentEvents-`<details>`), Voting-Buttons pro Spieler + Skip. `voting_result`-Toast mit Slide-In + Last-Words-Quote.                                                                                                                                                                                                                                                                        | 1.5 Tage | ⛔ TODO                                |
| 4.9   | **Endscreen** — finalSummary-bound: Awards-Liste, Per-Player-Stats (Tasks, Sabotagen, Coffee-final, Ability-used), AI-Postmortem `<pre>`-Block, Confetti-Particles. Reset-Button (host-only) sendet `return_to_lobby`.                                                                                                                                                                                                                                                   | 1 Tag    | ⛔ TODO                                |
| 4.10  | **Among-Us-Features** — Vents (V-Key + Animation + Sting), Take-Down (Proximity-Button + Cooldown), Body-Discovery + Report-Button, Lights-Vignette (radial alpha-overlay auf CanvasLayer), Comms-Disable (Sidebar-blank), Spectator-Mode (free-fly cam, only-ghosts-visible).                                                                                                                                                                                           | 5–8 Tage | 🟡 (Ghost-Alpha vorhanden)             |
| 4.11  | **Sound-Polish** — Footstep-Variants pro Floor-Material (kitchen-tile vs carpet vs concrete), BGM optional mit Mute-Toggle, mehr UI-Stings.                                                                                                                                                                                                                                                                                                                              | 1 Tag    | 🟡 (Footsteps Carpet-only)             |
| 4.12  | **Polish-Sweep** — Touch-Controls für Mobile (virtual joystick + Action-Buttons), Auto-Reconnect (rejoin-Message bei WS-DC), In-Game-ESC-Menü Audio-Slider, Bug-Fixes aus Live-Tests, FPS-Counter optional.                                                                                                                                                                                                                                                              | 3–5 Tage | ⛔ TODO                                |
