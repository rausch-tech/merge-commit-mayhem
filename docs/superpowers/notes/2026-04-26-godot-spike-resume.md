...
├── ROADMAP.md                  # MODIFIZIERT: neue Pre-Tier-3-Sektion vor Tier 3
└── superpowers/
...
1. **Godot lokal installieren.** Godot 4.3 LTS, nicht 4.4 (LTS-Argument für langen Tier-3-Sprint).
...
5. **`docs/ROADMAP.md` Pre-Tier-3 P3.4 auf `done` setzen.**
6. **Branch-Decision:** Wenn alles grün → `git push origin slice/godot-spike` + GitHub-PR, oder direkt `git merge slice/godot-spike` auf main, oder Branch liegen lassen bis Tier 3 startet.
...
- **Spike wird zur Tier-3-Basis:** Die Phasen 3.1–3.4 aus der Roadmap übernehmen die Strukturen aus `godot/scripts/` direkt (Protocol, WSClient, Main bleiben — DebugRenderer wird durch Tilemap + Charakter-Sprites ersetzt).
...
- **Godot-Version-Entscheidung:** 4.3 LTS, nicht 4.4 latest. Begründung: Tier 3 ist langer Sprint, LTS reduziert Breakage.
