...
def _room_with_roles() -> tuple[GameRoom, str, str]:
    """Return (room, chaos_player_id, dev_player_id). Uses seeded rng for
    determinism. Tier 2.7 (rework): apply_sabotage tests do their own
    per-sabotage snap via ``_trigger`` below, since each sabotage now binds
...
def _trigger(room: GameRoom, chaos_id: str, sabotage_id: str) -> None:
    """Snap chaos to the right themed-object anchor, then trigger. Mirrors
    actual gameplay: chaos walks to e.g. the CI console, then sabotages."""
    snap_to_object_for_sabotage(room, chaos_id, sabotage_id)
    room.apply_sabotage(chaos_id, sabotage_id)
...
def test_cannot_trigger_unknown_sabotage():
    ...
    snap_to_object_for_sabotage(room, chaos_id, "ci_cd_red")
    with pytest.raises(GameRoomError) as exc:
...
def test_chaos_must_stand_at_themed_object_to_trigger():
    """Tier 2.7 rework: sabotages from anywhere are forbidden — chaos has to
    walk to the matching themed object first (CI console for ci_cd_red etc.).
    """
    room, chaos_id, _ = _room_with_roles()
    # Park chaos far from any anchor.
...
def test_chaos_at_wrong_object_type_cannot_trigger():
    """Standing at the coffee machine doesn't let chaos trigger ci_cd_red."""
    room, chaos_id, _ = _room_with_roles()
    snap_to_object_for_sabotage(room, chaos_id, "coffee_outage")
    with pytest.raises(GameRoomError) as exc:
        room.apply_sabotage(chaos_id, "ci_cd_red")
    assert exc.value.code == "NOT_NEAR_OBJECT"


def test_chaos_can_trigger_when_inside_object_radius():
    """Walk to the CI console — allowed."""
    from app.game.tasks import SABOTAGE_OBJECT_INTERACTION_RADIUS

    room, chaos_id, _ = _room_with_roles()
    ci_anchor = next(a for a in room.map.task_anchors if a.object_type == "ci_console")
    # Just inside the radius.
    room.players[chaos_id].x = ci_anchor.x + SABOTAGE_OBJECT_INTERACTION_RADIUS - 1
    room.players[chaos_id].y = ci_anchor.y
...
def test_cooldown_prevents_retrigger():
    room, chaos_id, _ = _room_with_roles()
    _trigger(room, chaos_id, "ci_cd_red")
    with pytest.raises(GameRoomError) as exc:
        room.apply_sabotage(chaos_id, "ci_cd_red")
...
def test_coffee_outage_zeroes_coffee():
    ...
    _trigger(room, chaos_id, "coffee_outage")
    assert room.coffee_level == 0


def test_mandatory_meeting_sets_meeting_timer():
    room, chaos_id, _ = _room_with_roles()
    _trigger(room, chaos_id, "mandatory_meeting")
...
def test_cooldown_clamps_at_zero_and_allows_retrigger():
    ...
    # Snap back (tick movement may have drifted) and retrigger.
    _trigger(room, chaos_id, "ci_cd_red")
...
def test_public_state_exposes_sabotages_and_tasks():
    ...
    assert ci_cd["triggerObjectTypes"] == ["ci_console"]
    assert len(ci_cd["triggerAnchors"]) >= 1
    task_ids = {t["id"] for t in state["tasks"]}
    assert "fix_unit_tests" in task_ids
    # Each task carries its objectType for client-side proximity hints.
    fix_tests = next(t for t in state["tasks"] if t["id"] == "fix_unit_tests")
    assert fix_tests["objectType"] == "qa_terminal"
...
def test_merge_conflict_storm_drops_pipeline_and_raises_incidents():
    ...
    room.incidents = 10
    _trigger(room, chaos_id, "merge_conflict_storm")
...
def test_fake_customer_request_drops_release_progress():
    ...
    _trigger(room, chaos_id, "fake_customer_request")
...
def test_flaky_tests_only_raises_incidents():
    ...
    _trigger(room, chaos_id, "flaky_tests")
...
def test_lights_out_sets_lights_off_flag():
    ...
    _trigger(room, chaos_id, "lights_out")
...
def test_comms_outage_sets_comms_down_flag():
    ...
    _trigger(room, chaos_id, "comms_outage")
